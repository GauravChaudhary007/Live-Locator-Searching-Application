...
var TextboxList = new Class({
  ...
  initialize: function(element, options){
	...
	getValues: function(){
		var resultQuery = this.list.getChildren().map(function(el){
...
TextboxListBit.Editable = new Class({
  ...
  initialize: function(value, textboxlist, options){
    ...
    this.element = new Element('input', {type: 'text', 'class': this.typeprefix + '-input', id: 'input-search-bar',autocomplete: 'off', value: this.value ? this.value[1] : ''}).inject(this.bit);		
...
TextboxListBit.Box = new Class({
  ...
  initialize: function(value, textboxlist, options){
    this.parent(value, textboxlist, options);
		...
		this.bit.getChildren().addEvent('click', function(e){ e.stop(); fetchSearchResult();});
  }
